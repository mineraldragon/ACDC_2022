...
## Main Goal
While studying the social behavior of animals in a laboratory setting, a wealth of unstructured data is typically generated. These data, such as hours-long video and/or audio recordings, are tedious for humans to analyze manually. ACDC is a project that seeks to solve this problem for audio analysis - helping researchers automatically detect and classify animal calls in long recordings, and outputting the results in a convenient format.

## Overall Flow
To train, the user places training data for discrete call types in corresponding folders to form a "call dictionary", and then instructs ACDC to develop detection and classification models based on the training data.
To process recordings, the user places recordings in a pre-specified folder and tells ACDC to process them, resulting in files containing call labels being generated and placed in an output folder. 
...
## Most recent changes
- Removal of unnecessary features that were giving errors
- Fixed Joblib issue
